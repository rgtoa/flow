...
export function assembleRafaelData(
  ...
  return {
    ...
    accounts: {
      ...
      credit: { name: credit?.name ?? "Credit card",     balance: Number(credit?.balance ?? 0), use: credit?.is_active ?? false },
    },
...
export async function persistRafaelSetup(
  ...
  rows.push({ user_id: userId, account_key: "credit", name: data.accounts.credit.name, balance: data.accounts.credit.balance, account_type: "credit", is_active: data.accounts.credit.use, sort_order: 1 });
